...
### Changed
- **Full GitHub migration**: Module path switched from `codeberg.org/pivpav/glow` to `github.com/piv-pav/glow`.
- **`glow upgrade`** fetches latest GitHub tag and upgrades via `go install github.com/piv-pav/glow@<tag>`.

## [0.11.1] - 2026-07-21

### Changed
- **`glow upgrade`**: Prints manual migration instructions to GitHub. This binary (Codeberg) cannot auto-upgrade. One-time `go install github.com/piv-pav/glow@latest` required.
...
### Added
- **MCP server startup banner**: Prints version and transport to stderr on start (`GLOW MCP Server vX.Y.Z — STDIO` / `HTTP :PORT/mcp`)

## [0.10.1] - 2026-07-03

### Added
- **`glow mcp --port` flag**: Start MCP server over Streamable HTTP transport (e.g. `glow mcp --port 8080`). Endpoint at `/mcp`. Default remains stdio (unchanged).
...
### Added
- **`glow upgrade` command**: Checks latest version tag on Codeberg and upgrades the binary via `go install`.

## [0.9.5] - 2026-06-26

### Removed
- **`--filter` flag removed from `glow search`**: Duplicate of embedded `tag:` / `path:` filter syntax. Use `glow search "query tag:go path:folder/"` instead.

## [0.9.4] - 2026-06-26

### Added
- **Wikilink rewriting on `move`**: When an article is moved/renamed, all `[[oldName]]` references in other articles are automatically rewritten to `[[newName]]`. Best-effort — handles exact matches; `[[name#anchor]]` variants are not rewritten.
...
### Added
- **`glow append --tag` / `--untag`**: Add or remove tags when appending content. Tag-only appends (no `--content`/`--stdin`) are allowed, consistent with `update`.
